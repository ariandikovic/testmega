...
--------------------------------
 ...
 test program for RFM22 and ATMEGA16 P1 prototype
 ...
 data interface: RS232, 115200 bit/s, 8 bits data, no parity, 1 stop bit
...
#define USART_BAUDRATE 115200 //Baudrate definition
...
void setParam (void){
unsigned char i;
//getDIPvalue returns value on DIP switch, we need only SW 5,6,7,8
//if everything OFF, power on default remains

i= getDIPvalue();	//read DIP SW
i=i&0b11110000;	// leave upper 4 bits
i=i>>4;			//shift for 4 
...
//set TX mode, transmit packet with data 6A i 45...
if (str[0]=='1'){				
...
if (str[0]=='2'){					
	...
	while (PINB & 0x01)	;//wait button T1 to continue
...
if (i==1) {
		PORTB = PORTB | (1<< PB2);	//pin PB2 high, green LED OFF
		PORTB = (PORTB & (~(1<< PB1)));//pin PB1 low, red LED ON
		_delay_ms(20); //light for 20 ms
		PORTB = (PORTB | (1<< PB1));	//pin PB1 high, red LED OFF
		spiWrite (0x07,0x05);			//enable READY mode,RX on
		}
